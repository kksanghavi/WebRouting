...
import {GuiEvent} from "@magic-xpa/engine";
import {GuiCommand, CommandType, GuiInteractive} from "@magic-xpa/gui";
import {StorageAttribute, MagicProperties} from "@magic-xpa/utils";
import {NString, List} from "@magic-xpa/mscorelib";
  ...
  openEditDialog(id: string, rowId: string, dim: any): void {
    ...
    if (!this.task.getProperty(id, HtmlProperties.ReadOnly, rowId)) {
      if (dialog != null && this.task.isTableControl(id)) {
        ...
        const dialogRef = dialog.open(null, this.componentList.dialogs.get('TextEditDialogComponent'), {
          width: dim.width + 'px',
